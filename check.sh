...
#git clone https://github.com/catonrug/7zip-detect.git && cd 7zip-detect && chmod +x check.sh && ./check.sh
...
#calculate wich file we will look for version detect
case "$filename" in
*msi)
file=$(echo "_7z.exe")
;;
...
printf %s "$emails" | while IFS= read -r onemail
do {
python ../send-email.py "$onemail" "To Do List" "only $links download links found: 
$download "
} done
fi
